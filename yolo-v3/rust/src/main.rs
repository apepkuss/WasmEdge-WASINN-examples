...
fn main_entry() -> Result<()> {
    let image_file = "/Volumes/Dev/secondstate/me/WasmEdge-WASINN-examples/yolo-v3/rust/dog.jpeg";
    let model_xml_file = Path::new("");
    let model_bin_file = Path::new("");

    // preprocess image
    let image_blob = process_image(image_file)?;

    // infer
    infer(model_xml_file, model_bin_file, image_blob)?;
...
fn process_image(filename: impl AsRef<str>) -> Result<Mat> {
...
fn infer<P>(xml_file: P, bin_file: P, blob: Mat) -> Result<()>
where
    P: AsRef<Path>,
{
...
// fn run<P>(xml_file: P, bin_file: P) -> Result<()>
// where
//     P: AsRef<Path>,
// {
...
// fn image_to_tensor(path: impl AsRef<Path>, height: u32, width: u32) -> Vec<u8> {
//     let pixels = Reader::open(path.as_ref()).unwrap().decode().unwrap();
